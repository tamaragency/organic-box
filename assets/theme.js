...
(function ($) {
  ...
  */
  ...
  theme.viewport = {
    ...
    } // scrollTo: ($elem, scrollTop = -1, speed = 1000) => {
    //   if($elem && typeof($elem) === 'string'){
    //     $elem = $($elem);
    //   }
    //
    //   if(scrollTop === -1 && $elem && $elem.length){
    //     scrollTop = $elem.offset().top - theme.Nav().bar.height();
    //   }
    //
  ...
  $(function ($) {
    ...
    /* Footer section mobile expand-collapse functionality */ 
    $('.section-footer .section-footer__title').click(function(){
      var scr_with = $( window ).width();
      if(scr_with <= 768){
        if($(this).next('.expand-collapse-block').is(':visible')){
        	$(this).next('.expand-collapse-block').hide(500);
          $(this).removeClass('expanded');
        }else{
        	$(this).next('.expand-collapse-block').show(500);
          $(this).addClass('expanded');
        }
      }
    });
    
    $(".template-product .yotpo.bottomLine").click(function() {
      $('html, body').animate({
        scrollTop: $("#product-add-review").offset().top
      }, 2000);
    });
